...
public class AutoModeDemo
{
    ...
    public void columnStoreMode()
    {
        ...
        System.out.println("***************** Converting the table: " + table + " " +
                "from row mode to column mode ****************************");

        s = new Schema(cols);

        String colTable = table + "_COLUMN";
        context = new ExecutorContext(catalog, bufferPoolManager);
        catalog.createTable(colTable, s);
        ipn = new InsertPlanNode(values, catalog.getTable(colTable).getOid());
        ex = (InsertExecutor) ExecutorFactory.createExecutor(context, ipn, 1);

        TablePage insertCol;
        insertCol = ex.insertColStore(values, catalog, colTable);
        ex.insertColVals(values, catalog, colTable);
        ...
        readPlanNode = new SeqReadPlanNode(s, ce, catalog.getTable(colTable).getOid());
    ...
    private int rowStoreInsertCount = 0;
    private int colStoreInsertCount = 0;
    private int lowerThreshold = 0;
    private int upperThreshold = 0;

    private final int rowStoreUpdateCount = 0;
    private final int colStoreUpdateCount = 0;

    private final int rowStoreDeleteCount = 0;
    private final int colStoreDeleteCount = 0;
    ...
    {
        do{
            if (isBetween(rowStoreInsertCount, lowerThreshold, upperThreshold)) {
                System.out.println("********* Performing row store insertion: *****************");
                rowStoreMode();
                rowStoreInsertCount++;

            } else if (isBetween(colStoreInsertCount, lowerThreshold, upperThreshold)) {
                columnStoreMode();
                colStoreInsertCount++;
            }
        }while(colStoreInsertCount < 1);
    }
